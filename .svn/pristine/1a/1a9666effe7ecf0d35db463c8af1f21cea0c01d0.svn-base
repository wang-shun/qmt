...
PKG="com.lesports.msite.server"
NAME="OpMsiteWebServer"
# Default port.
CMD=""
SERVICE_NAME="qmt-op-msite-web"
...
function start() {
    java ${JAVA_OPTS} -cp ${PHOME}/server/classes:${PHOME}/server/lib/* ${PKG}.${NAME} ${PORT} >> ${STD_LOG} 2>&1 &
    echo $! > ${PID_FILE}
    pid=`cat ${PID_FILE}`
