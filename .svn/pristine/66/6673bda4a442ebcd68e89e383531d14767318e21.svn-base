...
public class NewsResource {
    ...
    public Map<String, Object> getRecommend(@QueryParam("type") @DefaultValue("0,1") String type,
											@QueryParam("newsType") @DefaultValue("0,1,2,3") String newsType,
    ...
    public List<NewsVo> getRelatedNews(@PathParam("id") long id,
        ...
        try {
			...
			if (page > 0 || count > 0) {
				...
				pageParam = PageUtils.createPageParam(0, 3);
			}
            return newsService.getRelatedNews4App(id, typeList, pageParam, callerBean.getCallerParam());
        } catch (Exception e) {
            LOG.error(e.getMessage(), e);
            throw new LeWebApplicationException(e.getMessage(), LeStatus.INTERNAL_SERVER_ERROR);
        }
     ...
     * @param isNeedRecommends   0: 不需要 1: 需要推荐新闻
