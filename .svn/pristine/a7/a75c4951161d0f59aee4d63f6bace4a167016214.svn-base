 ...
 */
@Path("/sms/v1")
public class PcEpisodeResource {

    private static final Logger LOG = LoggerFactory.getLogger(PcEpisodeResource.class);
    ...
    public DetailEpisodeVo getEpisodeById(@PathParam("id") long id,
        ...
        try {
            DetailEpisodeVo episode = episodeService.getEpisodeByIdRealtime(id, caller.getCallerParam());
            return episode;
    ...
    public List<HallEpisodeVo> getMultiEpisodesByIds(@QueryParam("ids") String ids,
        ...
        try {
            Preconditions.checkArgument(StringUtils.isNotBlank(ids), "请传入ids,多个已英文逗号隔开");
            return episodeService.getMultiEpisodesByIdsRealtime(ids, caller.getCallerParam());
        } catch (LeWebApplicationException e) {
            LOG.error(e.getMessage(), e);
            throw e;
        } catch (Exception e) {
