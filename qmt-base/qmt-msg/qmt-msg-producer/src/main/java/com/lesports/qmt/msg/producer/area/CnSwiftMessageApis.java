 ...
 */
public class CnSwiftMessageApis {
    private static final Logger LOG = LoggerFactory.getLogger(CnSwiftMessageApis.class);
    private static final String dest = "lesports.qmt.msg.cn.direct/lesports.routingkey.qmt.msg.cn"; //exchangeName/routingKeyName
    private static ExecutorService executorService = Executors.newFixedThreadPool(5);
    private static JmsTemplate leJmsTemplate;

    static {
        synchronized (CnSwiftMessageApis.class) {
            try {
                AMQConnectionFactory factory = new AMQConnectionFactory(MsgProducerConstants.CN_SWIFTQ_CONNECTION_URL);
