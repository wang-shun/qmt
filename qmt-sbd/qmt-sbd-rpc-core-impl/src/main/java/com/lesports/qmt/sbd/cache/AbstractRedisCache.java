package com.lesports.qmt.sbd.cache;
...
abstract public class AbstractRedisCache<T extends TBase, ID extends Serializable> implements LeCrudRepository<T, ID> {
    ...
    protected static final RedisOp redisOp = RedisClientFactory.getRedisClient("rpc_sbd_cache");
    ...
    private boolean deleteEntity(ID id) {
        ...
        String keyCibn = "CIBN_" + getKeyById(id);
        result &= getRedisOp().del(key) & getRedisOp().del(keyCibn);
//        deleteRedisCache(key);
//        deleteRedisCache(keyCibn);
        return result;
    }
