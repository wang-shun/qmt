...
public class TopicResource {
    ...
    public Map<String, Object> list(@QueryParam("cid") @DefaultValue("0") int channelId,
        ...
        try {
            List<TopicItem> items = Lists.newArrayList();
			CountryCode country = callerBean.getCallerParam().getCountry();
			if (country == CountryCode.US) {
				items = topicService.list(channelId, page, count);
			} else {
				items = topicService.searchCmsList(channelId, page, count, secondCg);
			}
