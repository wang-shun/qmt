...
public class MenuResource {
    ...
    public MenuVo getMenu(@PathParam("id") long id,
                          @BeanParam CallerBean callerBean) {
        try {
            TMenu tMenu = QmtConfigApis.getTMenuById(id);
            return menuVoCreater.createMenuVo(menuService.getMenuWithFallback(id, callerBean.getCallerParam()),callerBean.getCallerParam());
        } catch (Exception e) {
    ...
    @Path("/children/menus")
    @Produces({AlternateMediaType.UTF_8_APPLICATION_JSON})
    public Map<String, Object> getChildMenus(@QueryParam("menuId") long menuId,
                                             @QueryParam("resourceId") long resourceId,
                                             @BeanParam CallerBean callerBean) {
        Preconditions.checkArgument(menuId != 0, "请传入菜单id");
        Preconditions.checkArgument(resourceId != 0, "请传入resourceId");
        List<TMenuItem> tMenuItems = new ArrayList<>();
        try {
            TMenu tMenu = QmtConfigApis.getTMenuById(menuId);
            if(null != tMenu) {
                List<TMenuItem> tMenuItemList = tMenu.getItems();
                for (TMenuItem tMenuItem : tMenuItemList) {
                    if (tMenuItem.getLevel() == 1 && tMenuItem.getResourceId() == resourceId) {
                        tMenuItems = tMenuItem.getSubItems();
                    }
                }
            }
            return ResponseUtils.createPageResponse(1, menuVoCreater.convertToMenuItemVos(tMenuItems,callerBean.getCallerParam()));
        } catch (Exception e) {
            LOG.error(e.getMessage(), e);
            throw new LeWebApplicationException(e.getMessage(), LeStatus.INTERNAL_SERVER_ERROR);
        }
